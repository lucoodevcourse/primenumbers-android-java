...
public enum PrimeCheckerUtil {;
    ...
    public static boolean isPrimeLocal(final long i, final Supplier<Boolean> isCancelled, final IntConsumer publishProgress) { // optimized non-Async/local isPrime method
        if (i < 2) return false;
        if (i == 2) return true;
        if (i % 2 == 0) return false;
        final long sqrt = Math.round(Math.sqrt(i));
        for (long k = 3; k <= sqrt; k += 2) {
            if (isCancelled.get() || i % k == 0) return false;
    ...
    public static boolean isPrimeOrig(final long i, final Supplier<Boolean> isCancelled, final IntConsumer publishProgress) { // original isPrime, now used for Async execution
        if (i < 2) return false;
        final long half = i / 2;
